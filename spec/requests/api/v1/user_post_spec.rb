...
RSpec.describe 'user post request' do
  ...
  it 'returns an error if the user info is sent as query params' do
    ...
    post "/api/v1/users?email=#{user_params[:email]}&password=#{user_parmas[:password]}&password_confirmation=#{user_params[:password_confirmation]}", params: JSON.generate(user: user_params)

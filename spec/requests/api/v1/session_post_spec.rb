...
RSpec.describe 'sessions post request' do
  ...
  it 'returns an error if the email can\'t be found' do
    ...
    expect(errors[:errors][0]).to include('Invalid username or password')
  end

  it 'returns an error if the password is incorrect' do
    headers = {'CONTENT_TYPE' => 'application/json', 'ACCEPT' => 'application/json'}
    user = create(:user, passowrd: 'password123')
  ...
  it 'returns an error if the user info is sent as query params' do
    ...
    post "/api/v1/sessions?email=#{user_params[:email]}&password=#{user_params[:password]}", headers: headers, params: JSON.generate(user: login_params)
